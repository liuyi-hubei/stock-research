(function () {
  ...
  function renderHome() {
    ...
    document.body.innerHTML = `${header("ranking")}<main><section class="ranking-section dark-band" id="ranking"><div class="shell"><div class="page-kicker"><span>研究总览</span><span>${db.meta.updatedAt}</span></div><div class="page-intro"><p>把长期假设放在同一张桌面上比较。排名依据个股报告基准情景中的十年分红复投年化收益率，最多展示 10 家。</p></div>${overviewBand()}<div class="ranking-head"><span>排名 / 公司</span><span>相对位置</span><span>预期回报</span></div><div class="ranking-list">${rankingRows()}</div><p class="ranking-note">收益率来自固定情景模型，是研究假设的可比结果，不是实时交易信号。</p><a class="text-link page-switch" href="reports.html">浏览全部个股报告 <b>↗</b></a></div></section></main>${footer()}`;
  }

  function renderReports() {
    document.body.className = "reports-page";
    document.title = `个股报告 | ${db.meta.siteName}`;
    document.body.innerHTML = `${header("reports")}<main><section class="reports-section dark-band" id="reports"><div class="shell"><div class="page-kicker"><span>研究档案</span><span>${String(db.stocks.length).padStart(2, "0")} 家公司</span></div><div class="page-intro"><p>每家公司保留完整判断链：事实、预测、估值、五种视角、风险与反证。选择一份报告，进入完整研究记录。</p></div>${overviewBand()}<div class="report-list">${reportRows()}</div></div></section></main>${footer()}`;
  }
